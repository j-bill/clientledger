...
class DeviceFingerprintService
{
    ...
    private static function normalizeUserAgent(string $userAgent): string
    {
        ...
        $normalized = preg_replace('/(\d+\.\d+)\.\d+\.\d+/', '$1', $userAgent) ?? $userAgent;
        $normalized = preg_replace('/AppleWebKit\/\d+\.\d+\.\d+/', 'AppleWebKit', $normalized) ?? $normalized;
        $normalized = preg_replace('/Version\/[\d.]+/', '', $normalized) ?? $normalized;
